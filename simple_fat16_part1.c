...
const char *FAT_FILE_NAME_1="fat16_1.img";
const char *FAT_FILE_NAME_2="fat16_2.img";
pthread_rwlock_t rwlock = PTHREAD_RWLOCK_INITIALIZER;
pthread_mutex_t gMutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t seek_rw_lock=PTHREAD_MUTEX_INITIALIZER;
FILE* log_fd;
 ...
 */
void sector_read(FAT16* fat16_ins, unsigned int secnum, void *buffer)
{
  FILE* fd=fat16_ins->fd,*fd1=fat16_ins->fd1,*fd2=fat16_ins->fd2;
  if(fd1!=NULL&&fd2!=NULL){
    BYTE sec_buf0[BYTES_PER_SECTOR];
    BYTE sec_buf1[BYTES_PER_SECTOR];
    BYTE sec_buf2[BYTES_PER_SECTOR];
    int fixflag0=0,fixflag1=0,fixflag2=0;
    pthread_mutex_lock(&seek_rw_lock);
    fseek(fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fread(sec_buf0, BYTES_PER_SECTOR, 1, fd);
    fseek(fd1, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fread(sec_buf1, BYTES_PER_SECTOR, 1, fd1);
    fseek(fd2, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fread(sec_buf2, BYTES_PER_SECTOR, 1, fd2);
    pthread_mutex_unlock(&seek_rw_lock);
    for(int i=0;i<BYTES_PER_SECTOR;i++){
      BYTE FIX_DATA=(sec_buf0[i]&sec_buf1[i])|(sec_buf1[i]&sec_buf2[i])|(sec_buf0[i]&sec_buf2[i]);//AB+BC+AC
      if((sec_buf0[i]^sec_buf1[i])!=0&&(sec_buf0[i]^sec_buf2[i])!=0){
        sec_buf0[i]=FIX_DATA;
        fixflag0=1;
      }
      else if((sec_buf1[i]^sec_buf0[i])!=0&&(sec_buf1[i]^sec_buf2[i])!=0)
      {
        sec_buf1[i]=FIX_DATA;
        fixflag1=1;
      }
      else if((sec_buf2[i]^sec_buf0[i])!=0&&(sec_buf2[i]^sec_buf1[i])!=0){
        sec_buf2[i]=FIX_DATA;
        fixflag2=1;
      }
    }
    memcpy(buffer,sec_buf0,BYTES_PER_SECTOR);
    if(fixflag0){
      pthread_mutex_lock(&seek_rw_lock);
      fseek(fd, BYTES_PER_SECTOR * secnum, SEEK_SET);
      fwrite(sec_buf0,BYTES_PER_SECTOR,1,fd);
      pthread_mutex_unlock(&seek_rw_lock);
      fflush(fd);
    }
    if(fixflag1){
      pthread_mutex_lock(&seek_rw_lock);
      fseek(fd1, BYTES_PER_SECTOR * secnum, SEEK_SET);
      fwrite(sec_buf1,BYTES_PER_SECTOR,1,fd1);
      pthread_mutex_unlock(&seek_rw_lock);
      fflush(fd1);
    }
    if(fixflag2){
      pthread_mutex_lock(&seek_rw_lock);
      fseek(fd2, BYTES_PER_SECTOR * secnum, SEEK_SET);
      fwrite(sec_buf2,BYTES_PER_SECTOR,1,fd2);
      pthread_mutex_unlock(&seek_rw_lock);
      fflush(fd2);
    }
  }
 ...
 */
void sector_write(FAT16* fat16_ins, unsigned int secnum, const void *buffer)
{
  FILE* fd=fat16_ins->fd,*fd1=fat16_ins->fd1,*fd2=fat16_ins->fd2;
  if(fd1!=NULL&&fd2!=NULL){
    pthread_mutex_lock(&seek_rw_lock);
    ...
    fseek(fd1, BYTES_PER_SECTOR * secnum, SEEK_SET);
    fwrite(buffer,BYTES_PER_SECTOR,1,fd1);
    fseek(fd2, BYTES_PER_SECTOR * secnum, SEEK_SET);
 ...
 * @param imageFilePath_1 冗余镜像文件路径1
 * @param imageFilePath_2 冗余镜像文件路径2
 * @return FAT16*       存储元数据的FAT16结构指针
 */
FAT16 *pre_init_fat16(const char *imageFilePath,const char* imageFilePath_1,const char* imageFilePath_2)
{
  ...
  if(BackUp){
    ...
    FILE *fd1 = fopen(imageFilePath_1, "rb+");
    if (fd == NULL)
    {
      fprintf(stderr, "Missing FAT16 image file!\n");
      exit(EXIT_FAILURE);
    }
    FILE *fd2 = fopen(imageFilePath_2, "rb+");
    if (fd == NULL)
    {
      fprintf(stderr, "Missing FAT16 image file!\n");
      exit(EXIT_FAILURE);
  ...
  /* Reads the BPB */
  sector_read(fat16_ins, 0, &fat16_ins->Bpb);
...
WORD fat_entry_by_cluster(FAT16 *fat16_ins, WORD ClusterN)
{
  ...
  sector_read(fat16_ins, FatSecNum, &sector_buffer);
...
void first_sector_by_cluster(FAT16 *fat16_ins, WORD ClusterN, WORD *FatClusEntryVal, WORD *FirstSectorofCluster, BYTE *buffer)
{
  ...
  *FirstSectorofCluster = ((ClusterN - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector;
  ...
  sector_read(fat16_ins, *FirstSectorofCluster, buffer);
}
...
int find_root(FAT16 *fat16_ins, DIR_ENTRY *Root, const char *path, off_t *offset_dir)
{
  ...
  sector_read(fat16_ins, fat16_ins->FirstRootDirSecNum, buffer);
  ...
  for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
  {
    ...
    if (is_valid && Root->DIR_Attr == ATTR_ARCHIVE)   //question? this should add pathDepth==1 ?
    {
    ...
    {
      sector_read(fat16_ins, fat16_ins->FirstRootDirSecNum + RootDirCnt, buffer);
      RootDirCnt++;
    }
  }
...
int find_subdir(FAT16 *fat16_ins, DIR_ENTRY *Dir, char **paths, int pathDepth, int curDepth, off_t *offset_dir)
{
  ...
  for (uint i = 1; Dir->DIR_Name[0] != 0x00; i++)
  {
    ...
    if (i % 16 == 0)
    {
      ...
      {
        sector_read(fat16_ins, FirstSectorofCluster + DirSecCnt, buffer);
        DirSecCnt++;
      }
      else
                  ...
                  off_t offset, struct fuse_file_info *fi)
{
  ...
  fwrite("Begin-readdir-",14,1,log_fd);
  fwrite(path,strlen(path),1,log_fd);
  fwrite("-end\n",strlen("-end\n"),1,log_fd);
  fflush(log_fd);
  ...
  if (strcmp(path, "/") == 0)
  {
    ...
    sector_read(fat16_ins, fat16_ins->FirstRootDirSecNum, sector_buffer);
    ...
    for (uint i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      ...
      if(Root.DIR_Attr!=0xf&&Root.DIR_Name[0]!=0xe5){
        unsigned char* name=path_decode(Root.DIR_Name);
      ...
      if (i % 16 == 0 && i != fat16_ins->Bpb.BPB_RootEntCnt)
      {
        sector_read(fat16_ins, fat16_ins->FirstRootDirSecNum + RootDirCnt, sector_buffer);
        RootDirCnt++;
      }
  ...
  else  // 要查询的目录不是根目录
  {
    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      if(Dir.DIR_Attr!=0xf&&Dir.DIR_Name[0]!=0xE5){
        unsigned char* name=path_decode(Dir.DIR_Name);
        // printf("%s:%x\n",name,Dir.DIR_Attr);
        filler(buffer,name,NULL,0);
      }
      ...
      if (i % 16 == 0)
      {
        ...
        if (DirSecCnt < fat16_ins->Bpb.BPB_SecPerClus)
        {
          ...
          /*** BEGIN ***/
          sector_read(fat16_ins, FirstSectorofCluster + DirSecCnt, sector_buffer);
          DirSecCnt++;
          /*** END ***/
        }
               ...
               struct fuse_file_info *fi)
{
  ...
  char log_str[100];
  fseek(log_fd,0,SEEK_END);
  sprintf(log_str,"Begin-read-path:%s-size:%d-End\n\0",path,size);
  fwrite(log_str,strlen(log_str),1,log_fd);
  fflush(log_fd);
  pthread_mutex_unlock(&gMutex);
  printf("\n be lock\n\n");
  /*** BEGIN ***/
      off_t beginBytes=offset,endBytes=offset+size-1;
      int begin_clusternum=0,end_clusternum=0;
      begin_clusternum=beginBytes/(fat16_ins->ClusterSize);
      end_clusternum=endBytes/(fat16_ins->ClusterSize);
      int begin_sector_num,end_sector_num;
      begin_sector_num=(beginBytes%(fat16_ins->ClusterSize))/BYTES_PER_SECTOR;
      end_sector_num=(endBytes%(fat16_ins->ClusterSize))/BYTES_PER_SECTOR;
      int begin_sector_offset,end_sector_offset;
      begin_sector_offset=beginBytes%BYTES_PER_SECTOR;
      end_sector_offset=endBytes%BYTES_PER_SECTOR;
      DIR_ENTRY DIR;
      off_t dir_offset;
      ...
      int Cluster_cnt=0;
      //找到beginbytes的簇
      while(Cluster_cnt<begin_clusternum){
        if(0xffff==ClusterN){
          pthread_rwlock_unlock(&rwlock);
          return 0;// 
        }
      ...
      //end 找到beginbytes的簇
      int buf_size=0;
      int cur_clusternum=begin_clusternum;
      while (cur_clusternum<=end_clusternum)
      {
        FirstSectorofCluster=((ClusterN - 2) * fat16_ins->Bpb.BPB_SecPerClus) + fat16_ins->FirstDataSector;//获取该簇第一个扇区号
        if(cur_clusternum==begin_clusternum&&cur_clusternum==end_clusternum){ //begin和end在相同簇
          sector_read(fat16_ins,FirstSectorofCluster+begin_sector_num,sector_buffer);

          if(begin_sector_num==end_sector_num){ //begin end 在相同sector
            memcpy(buffer,(void*)sector_buffer+begin_sector_offset,size);
            buf_size+=size;
          }
          else{
            memcpy(buffer,(void*)sector_buffer+begin_sector_offset,BYTES_PER_SECTOR-begin_sector_offset);
            buf_size+=BYTES_PER_SECTOR-begin_sector_offset;
            for(int i=begin_sector_num+1;i<end_sector_num;i++){
              sector_read(fat16_ins,FirstSectorofCluster+i,sector_buffer);
              memcpy(buffer+buf_size,sector_buffer,BYTES_PER_SECTOR);
              buf_size+=BYTES_PER_SECTOR;
            }
            sector_read(fat16_ins,FirstSectorofCluster+end_sector_num,sector_buffer);
            memcpy(buffer+buf_size,sector_buffer,end_sector_offset+1);
            buf_size+=end_sector_offset+1;
          }
          pthread_rwlock_unlock(&rwlock);
          return buf_size;
        }
        else if(cur_clusternum==begin_clusternum){ //在读begin的簇
          sector_read(fat16_ins,FirstSectorofCluster+begin_sector_num,sector_buffer);
          memcpy(buffer+buf_size,sector_buffer+begin_sector_offset,BYTES_PER_SECTOR-begin_sector_offset);
          buf_size+=BYTES_PER_SECTOR-begin_sector_offset;
          for(int i=begin_sector_num+1;i<fat16_ins->Bpb.BPB_SecPerClus;i++){
            sector_read(fat16_ins,FirstSectorofCluster+i,sector_buffer);
            memcpy(buffer+buf_size,sector_buffer,BYTES_PER_SECTOR);
            buf_size+=BYTES_PER_SECTOR;
          }
        }
        else if(cur_clusternum==end_clusternum){//读end的簇
          for(int i=0;i<end_sector_num;i++){
            sector_read(fat16_ins,FirstSectorofCluster+i,sector_buffer);
            memcpy(buffer+buf_size,sector_buffer,BYTES_PER_SECTOR);
            buf_size+=BYTES_PER_SECTOR;
          }
          ...
          for(uint i=0;i<fat16_ins->Bpb.BPB_SecPerClus;i++){
            sector_read(fat16_ins,FirstSectorofCluster+i,sector_buffer);
            memcpy(buffer+buf_size,sector_buffer,BYTES_PER_SECTOR);
            buf_size+=BYTES_PER_SECTOR;
          }
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  char log_str[100];
  fseek(log_fd,0,SEEK_END);
  sprintf(log_str,"Begin-mknod-path:%s-End\n\0",path);
  fwrite(log_str,strlen(log_str),1,log_fd);
  fflush(log_fd);
  pthread_mutex_unlock(&gMutex);
  /** TODO:
...
int dir_entry_create(FAT16 *fat16_ins, int sectorNum, int offset, char *Name, BYTE attr, WORD firstClusterNum, DWORD fileSize)
{
  ...
  /*** BEGIN ***/
  BYTE sector_buffer[BYTES_PER_SECTOR];
  sector_read(fat16_ins,sectorNum,sector_buffer);
  memcpy(sector_buffer+offset,entry_info,BYTES_PER_DIR);
  sector_write(fat16_ins,sectorNum,sector_buffer);
  /*** END ***/
  free(entry_info);
  return 0;
}
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
   ...
   **/
  /*** BEGIN ***/
  write_fat_entry(fat16_ins,ClusterNum,0x0000);
  /*** END ***/
...
int fat16_unlink(const char *path)
{
  ...
  char log_str[100];
  fseek(log_fd,0,SEEK_END);
  sprintf(log_str,"Begin-unlink-path:%s-End\n\0",path);
  fwrite(log_str,strlen(log_str),1,log_fd);
  fflush(log_fd);
  pthread_mutex_unlock(&gMutex);
  ...
  BYTE sector_buffer[BYTES_PER_SECTOR];
  DWORD sectorNum;
  int offset, findFlag = 0, RootDirCnt = 1, DirSecCnt = 1;
  WORD FatClusEntryVal, FirstSectorofCluster;
...
int main(int argc, char *argv[])
{
  ...
  log_fd=fopen("./fat16.log","r+");
  FAT16 *fat16_ins = pre_init_fat16(FAT_FILE_NAME,FAT_FILE_NAME_1,FAT_FILE_NAME_2);
